...
namespace GPU_HeiProMap {
    ...
    inline LargeVertexPartitionCSR rebuild_scratch(const Graph &device_g,
        ...
        TIME("LargeVertexPartitionCSR", "rebuild_scratch", "init",
        ...
        TIME("LargeVertexPartitionCSR", "rebuild_scratch", "mark",
        ...
        TIME("LargeVertexPartitionCSR", "rebuild_scratch", "count",
        ...
        TIME("LargeVertexPartitionCSR", "rebuild_scratch", "set_rows",
        ...
        TIME("LargeVertexPartitionCSR", "rebuild_scratch", "init_rest",
        ...
        TIME("LargeVertexPartitionCSR", "rebuild_scratch", "fill",
             Kokkos::parallel_for("fill", device_g.n, KOKKOS_LAMBDA(const vertex_t u) {
                 ...
                 if (csr.ids(j) == v_id) { csr.weights(j) += w; break; }
                 j += 1;
                 }
                 }
    ...
    inline LargeVertexPartitionCSR rebuild(const Graph &device_g,
        ...
        TIME("LargeVertexPartitionCSR", "rebuild", "init_row",
             LargeVertexPartitionCSR csr;
             csr.row = DeviceU32("row", device_g.n + 1);
             Kokkos::deep_copy(csr.row, 0);
             Kokkos::fence();
        ...
        TIME("LargeVertexPartitionCSR", "rebuild", "set_rows",
             Kokkos::parallel_scan("set_rows", device_g.n, KOKKOS_LAMBDA(const vertex_t u, u32 &carry, const bool final) {
                 const u32 c = counts(u);
                 if (final) csr.row(u + 1) = carry + c; // write inclusive, row[0] already 0
                 carry += c;
        ...
        TIME("LargeVertexPartitionCSR", "rebuild", "init_rest",
             const u32 words = (p_manager.k + WORD_BITS - 1) / WORD_BITS;
        ...
        TIME("LargeVertexPartitionCSR", "rebuild", "fill",
             Kokkos::parallel_for("fill", device_g.n, KOKKOS_LAMBDA(const vertex_t u) {
                 u32 n_beg = device_g.neighborhood(u);
                 u32 n_end = device_g.neighborhood(u + 1);
                 // u32 n_len = n_end - n_beg;
                 ...
                 for (u32 t = 0; t < r_len; t++) {
                 if (j == r_end) { j = r_beg; }
                 if (csr.ids(j) == p_manager.k) { csr.ids(j) = v_id; csr.weights(j) = w; break ; }
                 if (csr.ids(j) == v_id) { csr.weights(j) += w; break ; }
                 j += 1;
                 }
                 }
    ...
    inline void move(LargeVertexPartitionCSR &csr,
                     ...
                     const DeviceU32 &to_move,
                     const DevicePartition &preferred) {
        TIME("LargeVertexPartitionCSR", "move", "init_bit_array",
             const u32 words = (p_manager.k + WORD_BITS - 1) / WORD_BITS;

             Kokkos::deep_copy(csr.bit_array, 0);
        );

        TIME("LargeVertexPartitionCSR", "move", "remove_weight",
             Kokkos::parallel_for("remove_weight", device_g.m, KOKKOS_LAMBDA(const u32 i) {
                 ...
                 if (old_w == w) { csr.ids(j) = p_manager.k; return; }
                 }
                 j += 1;
                 }
                 });
        ...
        TIME("LargeVertexPartitionCSR", "move", "add_conn",
        ...
        TIME("LargeVertexPartitionCSR", "move", "count_and_sum",
             DeviceU32 counts("counts", device_g.n);
             u32 sum = 0;
        ...
        if (sum > 0) {
            ...
            TIME("LargeVertexPartitionCSR", "move", "sum_counts",
        ...
        TIME("LargeVertexPartitionCSR", "move", "add_weight",
             Kokkos::parallel_for("add_weight", device_g.m, KOKKOS_LAMBDA(const u32 i) {
                 ...
                 for (u32 t = 0; t < r_len; t++) {
                 if (j == r_end) { j = r_beg; }
                 if (csr.ids(j) == new_u_id) { Kokkos::atomic_add(&csr.weights(j), w); return; }
                 j += 1;
                 }
                 });
             Kokkos::fence();
